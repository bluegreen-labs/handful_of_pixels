...
Below I wrap the steps as defined in @sec-phenology-estimates in a single function which takes a data frame as input, a phenological phase and threshold value (0 - 1) as parameters.
...
# plot maps + comparisons
par(mfrow=c(c(2,2)))
plot(phenology_map, main = "Custom algorithm", zlim = c(0,180))
plot(phenology_raster, main = "MCD12Q2 MODIS product", zlim = c(0,180))
plot(abs(phenology_raster - phenology_map), main = "Absolute error (MODIS vs. custom)")

# add scatterplot
plot(as.matrix(phenology_map), as.matrix(phenology_raster), xlab = "MODIS", ylab = "custom")
abline(0,1, col = "red")
