...
r <- readRDS(here::here("data/lai_2012.rds")) |>
  filter(
    band == "Lai_500m"
  )
```
...
With the function defined we can now apply this function to all pixels, and along the time axis (layers) of our `terra` raster stack. The `app()` function allows you to do exactly this!
...
pheno <- MODISTools::mt_to_terra(pheno)
```
