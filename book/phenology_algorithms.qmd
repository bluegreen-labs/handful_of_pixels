...
Smoothing can be achieved using various algorithms, such as splines, loess regressions. In this case, I will use the savitsky-golay algorithm, a common algorithm used across various vegetation phenology products. Luckily the methodology is already implemented in the `signal` package.
...
Note that the `sgolayfilt()` in `signal` does not allow for NA values to be present to function properly. As such, we operate on the full data set at its original time step. Since data is only provided at a 8-day time interval you would be limited to this time resolution in determining final phenology metrics.

To get estimates close to a daily time step we need to expand the data to a dialy time-step and merge the original smoothed data.
