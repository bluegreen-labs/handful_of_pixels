...
These methods are mostly applied to various vegetation indices, which link spectral characteristics as measured by satellites to the physical development of the canopy [@Myneni1995; @Huete2002]. For example, the Normalized Vegetation Index (NDVI) uses both the red and near-infrared spectral domain to sense the state of the vegetation, while the Enhanced Vegetation Index (EVI, @fig-threshold) does the same while accounting for the saturation of the signal over dense canopies. This multi-spectral nature of satellite remote sensing data, as well as their temporal dynamics, can be leveraged using a veritable zoo of indices tailored to specific properties and research needs [@zeng_optical_2022]. An in depth analysis on how to use this multi-spectral data in a different context is given in @sec-land-cover .
...
Smoothing can be achieved using various algorithms, such as splines, loess regressions, etc. In this case, I will use the [Savitsky-Golay filter](https://en.wikipedia.org/wiki/Savitzky%E2%80%93Golay_filter), a common algorithm used across various vegetation phenology products. Luckily the methodology is already implemented in the `signal` package.
