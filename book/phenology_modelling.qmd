...
gdd_model <- function(data, par) {
  ...
  return(which(gdd >= accumulated_days)[1])
}
```
...
# returns the RMSE
rmse_gdd <- function(data, par) {
  
 # grab observed values
 obs <- as.vector(data$observed)
 
 # calculate phenology predictions
 pred <- as.vector(gdd_model(par = par, data = data))
